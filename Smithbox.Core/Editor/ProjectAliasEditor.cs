...
public static class ProjectAliasEditor
{
    ...
    public static void Draw()
    {
        ...
        if (Display)
        {
            if (ImGui.Begin("Project Aliases##projectAliasWindow", ref Display, ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoMove))
            {
                Shortcuts();
    ...
    private static void DisplayAliasEntryList()
    {
        ...
        for(int i = 0; i < source.Count; i++)
        {
            ...
            if(display)
            {
                var displayName = StringUtils.TruncateWithEllipsis($"{entry.ID}:{entry.Name}", 42);
    ...
    {
        var projectFolder = $@"{TargetProject.ProjectPath}\.smithbox\Assets\Aliases\{LocatorUtils.GetGameDirectory(TargetProject.ProjectType)}";
