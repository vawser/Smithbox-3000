...
namespace Smithbox.Core.FileBrowserNS.Entries
{
    public abstract class BndFsEntry : SoulsFileFsEntry
    {
        ...
        internal void SetupChildren(Project ownerProject, BinderReader bnd)
        {
            ...
            foreach (var file in bnd.Files)
            {
                if (ImGui.CollapsingHeader(file.Name))
                {
                    ...
                    PropertyTable($"{file.Name}##table", (row) =>
                    {
                        ...
                        row("Flags", StringUtils.FlagsEnumToString(file.Flags));
    ...
    public class BndFileFsEntry : FsEntry
    {
        ...
        public BndFileFsEntry(Project ownerProject, string name, Func<Memory<byte>> getDataFunc)
        {
            this.name = name;
            this.getDataFunc = getDataFunc;
